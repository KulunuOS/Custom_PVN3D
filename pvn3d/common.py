...
class Config:
    def __init__(self, dataset_name='ycb', cls_type=''):
        ...
        elif self.dataset_name == 'Adapt':

            print("inside Adapt <<<" + str(dataset_name))
            self.n_objects = 3 + 1
            self.n_classes = 3 + 1
            self.Adapt_cls_lst_p = os.path.abspath(
                os.path.join(
                    self.exp_dir, 'datasets/Adapt/dataset_config/classes.txt'
                )
            ) 
            self.Adapt_root = os.path.abspath(
                os.path.join(
                    self.exp_dir, 'datasets/Adapt/Adapt_dataset'
                )
            )

            self.Adapt_kps_dir = os.path.abspath(
                os.path.join(
                    self.exp_dir, 'datasets/Adapt/Adapt_object_kps'
                )
            )
            Adapt_r_lst_p = os.path.abspath(
                os.path.join(
                    self.exp_dir, 'datasets/Adapt/dataset_config/radius.txt'
                )
            )
            self.use_preprocess = True
            self.Adapt_r_lst = list(np.loadtxt(Adapt_r_lst_p))
            self.Adapt_cls_lst = self.read_lines(self.Adapt_cls_lst_p)
            self.Adapt_sym_cls_ids = [1, 2, 3]
            self.Adapt_test_pkl_p = os.path.join(
                self.exp_dir,
                'datasets/Adapt/test_val_data_pts{}.pkl'.format(self.n_sample_points)
            )        
        ...
        self.intrinsic_matrix = {
            ...
            'ycb_K2': np.array([[1077.836, 0.        , 323.7872],
                                [0.      , 1078.189  , 279.6921],
                                [0.      , 0.        , 1.0]], np.float32),
            'Adapt': np.array([[554.25469119, 0.,         320.5],
                                [0.,        554.25469119,  240.5],
                                [0.,        0.,         1.]]),
